# Runs an SVM and a RF excluding each category of features one at a time
# Outputs a CSV for each classifier with the accuracy, precision, recall, and f-score for each cat
...
# Returns a dictionary of categories (key = cat, vals = feats in cat)
def get_features(features_csv, lg):
    ...
    cat_dict = {} # dictionary of each category of feature
...
def main():
    ...
    svm = pd.DataFrame(svm, columns = ['cat', 'acc', 'pB', 'pM', 'pC', 'rB', 'rM', 'rC', 'fB', 'fM', 'fC', 'n feat'])
    rf = pd.DataFrame(rf, columns = ['cat', 'acc', 'pB', 'pM', 'pC', 'rB', 'rM', 'rC', 'fB', 'fM', 'fC', 'n feat'])
    svm.to_csv(path + "-abl-cat-SVM.csv")
    rf.to_csv(path + "-abl-cat-RF.csv")
