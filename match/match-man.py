...
# Praat file
with open("/Users/Laura/Desktop/Dissertation/data/mandarin/mandarin-praat.txt") as praat:
...
# VS thirds file
with open("/Users/Laura/Desktop/Dissertation/data/mandarin/mandarin-vs-3.txt") as vs3:
	reader = csv.reader(vs3, delimiter = '\t')
	vs3header = next(reader)
	for line in reader:
		measures = []
		if line[1] == "01" or line[1] == "02" or line[1] == "03": # Exclude error/4th tone
			filename = line[0][:-4]
...
w = csv.writer(open("/Users/Laura/Desktop/Dissertation/data/mandarin/man-all.csv", "w"))
	...
	if praatPhon == vs3Phon:
		for i in range(len(header)): # Go through each column
			if i not in skip: # If it's not a column to skip
				if header[i] == "phonation":
					if line[i] == "01" or line[i] == "02":
						line[i] = "M"
					if line[i] == "03":
						line[i] = "C"
