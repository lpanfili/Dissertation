...
def runClass(x, y):
    ...
    for train_index, test_index in skf.split(x,y):
        ...
        predictedy = cross_val_predict(clf, x_test, y_test)
        #print(predictedy)
        p, r, f, s = prfs(y_test, predictedy)
        p = p * 100
        r = r * 100
        f = f * 100
        precList.append(p)
    ...
    prec_all.append(str(round(np.mean(precList),3)))
    recall_all.append(str(round(np.mean(recallList),3)))
    fscore_all.append(str(round(np.mean(fscoreList),3)))
